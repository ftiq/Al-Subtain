...
class WebsiteAppointmentProducts(WebsiteAppointment):
    ...
    def _handle_appointment_form_submission(
        ...
        if sale_lines:
            _logger.info(f"Products to add: {len(sale_lines)} items")
        else:
            _logger.info("No products selected for this appointment")
        ...
        if not sale_lines:
            ...
            try:
                if order_tmp and answer_input_values:
                    ...
                    try:
                        if order_tmp:
                            order_tmp.sudo().write({'appointment_datetime': date_start})
                    except Exception as dt_err:
                        _logger = logging.getLogger(__name__)
                        _logger.warning(f"Failed to write appointment_datetime (no sale lines): {dt_err}")
        ...
        for product_id, qty in sale_lines:
            try:
                # البحث عن المنتج الموجود في الطلب
                existing_line = order.order_line.filtered(lambda l: l.product_id.id == product_id)
                if existing_line:
                    # إذا كان المنتج موجوداً، تحديث الكمية بدلاً من إضافة سطر جديد
                    existing_line[0].sudo().write({'product_uom_qty': qty})
                else:
                    # إضافة المنتج الجديد باستخدام _cart_update للتعامل مع التسعير والضرائب
                    order.sudo()._cart_update(product_id=product_id, add_qty=qty)
            except Exception as e:
                # في حالة فشل _cart_update، إضافة السطر يدوياً
                existing_line = order.order_line.filtered(lambda l: l.product_id.id == product_id)
                if existing_line:
                    existing_line[0].sudo().write({'product_uom_qty': qty})
                else:
                    # إنشاء سطر جديد يدوياً
        ...
        try:
            if order:
                order.sudo().write({'appointment_datetime': date_start})
                _logger.info(f"Set appointment_datetime for order {order.id}: {date_start}")
        except Exception as dt_err:
            _logger = logging.getLogger(__name__)
            _logger.warning(f"Failed to write appointment_datetime: {dt_err}")

        # معالجة الملفات المرفقة إن وجدت
        try:
            if order and hasattr(request, 'httprequest') and request.httprequest.files:
                uploaded_files = request.httprequest.files.getlist('appointment_attachments')
                if uploaded_files:
                    ...
                    if attachment_ids:
                        order.message_post(
                            body=_('تم رفع مرفقات مع الموعد.'),
            ...
            _logger = logging.getLogger(__name__)
            _logger.warning(f"Failed to process appointment attachments: {attach_err}")
    ...
    def appointment_form_submit(self, appointment_type_id, datetime_str, duration_str, name, phone, email, staff_user_id=None, available_resource_ids=None, asked_capacity=1,
        ...
        _logger = logging.getLogger(__name__)
        _logger.info(f"Processing appointment for: {name}")
        
        # البحث عن معايير المنتجات
        product_params = {k: v for k, v in request.params.items() if 'product_qty_' in k}
        if product_params:
            _logger.info(f"Product params found: {len(product_params)} items")
        ...
        # معالجة التوقيت بشكل صحيح مع المنطقة الزمنية
        timezone = request.session.get('timezone') or appointment_type.appointment_tz
        
        # إذا لم تكن المنطقة الزمنية محددة، استخدم UTC كافتراضي آمن
        if not timezone:
            timezone = 'UTC'
            _logger.warning(f"No timezone found, using UTC as fallback")
        ...
        try:
            ...
            _logger.error(f"Unknown timezone: {timezone}, falling back to UTC")
            tz_session = pytz.UTC
            timezone = 'UTC'
        datetime_str = unquote_plus(datetime_str)
        duration = float(duration_str)
        
        # تحويل التوقيت إلى UTC بشكل متين، مع دعم سلاسل ISO التي تحتوي على إزاحة زمنية
        date_start = None
        try:
            parsed_dt = du_parser.isoparse(datetime_str)
        except Exception:
            parsed_dt = None
        
        if parsed_dt and parsed_dt.tzinfo:
            # السلسلة تحتوي على معلومات المنطقة الزمنية بالفعل (مثل Z أو +03:00)
            date_start = parsed_dt.astimezone(pytz.utc).replace(tzinfo=None)
        else:
            # لا توجد معلومات منطقة زمنية في السلسلة، اعتبرها بالمنطقة المختارة ثم حوّل إلى UTC
            try:
                local_naive = parsed_dt if parsed_dt else fields.Datetime.from_string(datetime_str)
                localized = tz_session.localize(local_naive)
                date_start = localized.astimezone(pytz.utc).replace(tzinfo=None)
            except Exception:
                # كملاذ أخير، تعامل معها كـ UTC دون تغيير (لتجنب إزاحة مزدوجة غير مقصودة)
                date_start = (parsed_dt or fields.Datetime.from_string(datetime_str)).replace(tzinfo=None)
        
        date_end = date_start + relativedelta(hours=duration)
        
        # تسجيل للتشخيص
        _logger.info(f"Timezone conversion: raw='{datetime_str}', tz='{timezone}' -> UTC: {date_start}")
        

        # تحضير المعايير للتحقق من صحة الموعد
        check_kwargs = {k: v for k, v in kwargs.items() if k not in ['duration']}
        resource_selected_id = check_kwargs.pop('resource_selected_id', None)
        
        if not self._check_appointment_is_valid_slot(appointment_type, staff_user_id, resource_selected_id, available_resource_ids, datetime_str, duration, asked_capacity, **check_kwargs):
        ...
        # تبسيط booking_line_values لتجنب خطأ الحقول غير الصحيحة
        booking_line_values = []
        # تعليق معالجة الموارد مؤقتاً حتى نحل مشكلة الحقول
        # if appointment_type.schedule_based_on == 'resources' and available_resource_ids:
        #     try:
        #         decoded_resource_ids = unquote_plus(available_resource_ids) if available_resource_ids else '[]'
        #         resource_ids = json.loads(decoded_resource_ids) if decoded_resource_ids and decoded_resource_ids != '[]' else []
        #         
        #         if resource_ids:
        ...
        #         _logger.warning(f"Failed to parse available_resource_ids: {available_resource_ids}, error: {e}")
        #         booking_line_values = []
