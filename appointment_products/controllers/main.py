...
class WebsiteAppointmentProducts(WebsiteAppointment):
    ...
    def _handle_appointment_form_submission(
        ...
        if date_start and hasattr(date_start, 'tzinfo') and date_start.tzinfo is None:
            try:
                user_tz = request.env.user.tz or request.httprequest.cookies.get('tz') or 'Asia/Baghdad'
            except:
                user_tz = 'Asia/Baghdad'  
            local_tz = pytz.timezone(user_tz)
            localized_date = local_tz.localize(date_start)
            date_start = localized_date.astimezone(pytz.UTC).replace(tzinfo=None)
            

            _logger = logging.getLogger(__name__)
            _logger.info(f"_handle_appointment_form_submission - تحويل التوقيت: ({user_tz}) -> UTC {date_start}")
    ...
    def appointment_form_submit(self, appointment_type_id, datetime_str, duration_str, name, phone, email, staff_user_id=None, available_resource_ids=None, asked_capacity=1,
        ...
        date_start_original = datetime.strptime(datetime_str, dtf)
        ...
        if date_start_original.tzinfo is None:
            localized_date = local_tz.localize(date_start_original)
            date_start = localized_date.astimezone(pytz.UTC).replace(tzinfo=None)
            
            _logger = logging.getLogger(__name__)
            _logger.info(f"تحويل التوقيت: الأصلي {date_start_original} ({user_tz}) -> UTC {date_start}")
        else:
