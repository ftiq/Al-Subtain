...
class WebsiteAppointmentProducts(WebsiteAppointment):
        ...
        try:
            if order and answer_input_values:
                ...
                for val in answer_input_values:
                    q_id = val.get('question_id')
                    if not q_id:
                        continue
                    if val.get('value_answer_id'):
                        ans = request.env['appointment.answer'].sudo().browse(val['value_answer_id']).name
                    elif val.get('value_text_box'):
                        ans = val['value_text_box']
                    else:
                        ans = ''
                    answer_map.setdefault(q_id, []).append(ans)

                body_parts = []
                for question in appointment_type.question_ids:
                    answers = answer_map.get(question.id) or []
                    if question.question_type in ['checkbox']:
                        answer_text = ', '.join(answers) if answers else _('N/A')
                    else:
                        answer_text = answers[0] if answers else _('N/A')
                ...
                if body_parts:
                    ...
                    fallback_parts = []
                    for question in appointment_type.question_ids:
                        param_key_prefix = f"question_{question.id}"
                        fallback_answer = ''
                        if question.question_type in ['char', 'text', 'select', 'radio']:
                            fallback_answer = request.params.get(param_key_prefix, '')
                            if question.question_type in ['select', 'radio'] and fallback_answer.isdigit():
                                fallback_answer = request.env['appointment.answer'].sudo().browse(int(fallback_answer)).name
                        elif question.question_type == 'checkbox':
                            checkbox_vals = [v for k, v in request.params.items() if k.startswith(param_key_prefix + '_answer_')]
                            fallback_answer = ', '.join(checkbox_vals)

                        fallback_answer = fallback_answer or _('N/A')
                        fallback_parts.append(f"<p><b>{question.name}:</b> {fallback_answer}</p>")

                    if fallback_parts:
                        order.message_post(body="".join(fallback_parts), message_type='comment', subtype_xmlid='mail.mt_comment')
    ...
    def upload_attachment(self, order_id, **post):
        ...
        if attachment_ids:
            order.message_post(body=_('تم رفع مرفقات.'), attachment_ids=attachment_ids, message_type='comment', subtype_xmlid='mail.mt_comment')
    ...
    def _get_customer_partner(self):
        ...
        # 4) Fallback: always return a real partner (never public user)
