...
class AppointmentTypeProduct(models.Model):
    ...
    price = fields.Float(
        string='السعر',
        compute='_compute_sample_fixed_price',
        readonly=True,
        store=True
    )
    ...
    @api.depends('product_id', 'product_id.related_sample_product_id')
    def _compute_sample_fixed_price(self):
        """حساب السعر من إعدادات العينة الثابتة أو استخدام السعر القياسي"""
        ...
        for record in self:
            ...
            sample_product = record.product_id.product_tmpl_id.related_sample_product_id
            if not sample_product:
                continue
            
            param_key = f'appointment_products.fixed_price_{sample_product.id}'
            fixed_price_val = ICP.get_param(param_key)
            
            if not fixed_price_val:
                continue
                
            try:
                fixed_price = float(fixed_price_val)
                if fixed_price > 0:
                    record.price = fixed_price
                    logging.getLogger(__name__).debug(
                        f'تم تطبيق السعر الثابت {fixed_price} للمنتج {record.product_id.name}'
                    )
            except (ValueError, TypeError) as e:
                logging.getLogger(__name__).warning(
                    f'خطأ في تحويل السعر الثابت {fixed_price_val} للمنتج {record.product_id.name}: {e}'
                )

    @api.onchange('product_id')
    def _onchange_product_update_price(self):
        """تحديث السعر تلقائياً عند تغيير المنتج"""
        if self.product_id:

            self._compute_sample_fixed_price()
