...
class ProjectTask(models.Model):
    ...
    main_sample_is_masonry = fields.Boolean(
        string='العينة الرئيسية طابوق',
        compute='_compute_main_sample_is_masonry',
        store=True,
        help='يحدد ما إذا كان النموذج الرئيسي في المهمة هو طابوق'
    )

    main_sample_is_concrete_core = fields.Boolean(
        string='العينة الرئيسية كور خرساني',
    ...
    def _compute_main_sample_is_masonry(self):
        """تحديد ما إذا كانت العينة الرئيسية طابوق"""
        for task in self:
            masonry_type = self.env.ref('appointment_products.masonry_type', raise_if_not_found=False)
            if masonry_type and task.form_line_ids:

                task.main_sample_is_masonry = any(
                    line.sample_type_id == masonry_type for line in task.form_line_ids
                )
            else:
                task.main_sample_is_masonry = False

    @api.depends('form_line_ids.product_id.product_tmpl_id.sample_type_id')
    def _compute_main_sample_is_concrete_core(self):
        """تحديد ما إذا كانت العينة الرئيسية كور خرساني"""
        for task in self:
            concrete_core_type = self.env.ref('appointment_products.concrete_core_sample_type', raise_if_not_found=False)
            if concrete_core_type and task.form_line_ids:
                task.main_sample_is_concrete_core = any(
                    line.sample_type_id == concrete_core_type for line in task.form_line_ids
                )
            else:
