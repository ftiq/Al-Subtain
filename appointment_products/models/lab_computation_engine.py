    ...
    def execute_formula(self, formula, variables, series_data=None):
        ...
        try:
            ...
            if 'result =' in formula:
                ...
                return float(safe_dict.get('result', 0))
            else:
                result = eval(formula, safe_dict)
                return float(result)
                
        except Exception as e:
            _logger.error(_("Error executing formula: %s, Error: %s"), formula, str(e))
            raise UserError(_('Error in formula calculation: %s') % str(e))
        ...
        def mode(values):
            if isinstance(values, (list, tuple)) and values:
                try:
                    ...
                    return values[0]  # إذا لم يوجد منوال واضح
            return values
