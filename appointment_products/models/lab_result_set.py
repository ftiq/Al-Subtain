...
class LabResultSet(models.Model):
    ...
    def _compute_statistics(self):
        """حساب إحصاءات التقدم بطريقة محسّنة مع تحديث تلقائي لحقل is_filled"""
        for record in self:
    ...
    def _compute_overall_result(self):
        """حساب النتيجة العامة - الفحص يفشل فقط إذا فشل معيار حرج"""
    ...
    def _compute_overall_conformity(self):
        """حساب حالة المطابقة العامة - تفشل فقط إذا فشل معيار حرج"""
        for result_set in self:
            filled_lines = result_set.result_line_ids.filtered('is_filled')
            if not filled_lines:
    ...
    def action_refresh_progress(self):
        """🔄 إعادة حساب نسبة التقدم والإحصاءات يدوياً"""
        for record in self:
            record.result_line_ids._compute_is_filled()
            ...
            record.message_post(
                body=f"تم تحديث نسبة التقدم: {record.progress_percentage:.2%} ({record.completed_criteria}/{len(record.result_line_ids)} معايير مكتملة)",
                message_type='notification'
            )
    ...
    def action_show_test_details(self):
        ...
        if details['critical_criteria']['total'] > 0:
            ...
            if details['critical_criteria']['failed'] > 0:
                message_lines.append(f"⚠️ معايير حرجة فاشلة: {', '.join(details['critical_criteria']['failed_names'])}")
        ...
        if details['non_critical_criteria']['total'] > 0:
            ...
            if details['non_critical_criteria']['failed'] > 0:
                message_lines.append(f"⚠️ معايير غير حرجة فاشلة: {', '.join(details['non_critical_criteria']['failed_names'])}")
...
class LabResultLine(models.Model):
    ...
    @api.depends('sample_no', 'test_type')
    def _compute_sample_identifier(self):
        for line in self:
            if line.test_type == 'computed':
