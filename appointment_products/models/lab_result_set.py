...
class LabResultSet(models.Model):
    ...
    reference_general_limit = fields.Float(
        related='sample_id.task_id.reference_general_limit',
    ...
    def _compute_days_remaining(self):
        ...
        for record in self:
            # إذا بدأ الفحص أو انتهى، لا تغير القيمة
            if record.state in ('in_progress', 'calculated', 'review', 'approved', 'completed'):
                # الاحتفاظ بالقيمة الحالية (باقية على القيمة المخزنة)
                continue
            ...
            # حدد عدد الأيام المطلوبة بناء على نوع الفحص
            if record.required_age_days == '7':
                target_days = 7
            elif record.required_age_days == '28':
                target_days = 28
            else:  # reserve
                target_days = 28  # افتراض 28 يوم للاحتياط
                
            # حساب تاريخ الفحص المطلوب
            from datetime import timedelta
            target_date = record.casting_date + timedelta(days=target_days)
            today = fields.Date.today()
            
            # حساب الأيام المتبقية أو المتجاوزة
            remaining = (target_date - today).days
            
            if remaining >= 0:
                # لا يزال هناك وقت متبقي أو اليوم هو موعد الفحص
                record.days_remaining = remaining
                record.factor = 0
            else:
                # تم تجاوز الموعد المحدد
                record.days_remaining = 0
                record.factor = abs(remaining)  # عدد الأيام المتجاوزة (قيمة موجبة)
            ...
            try:
                if result_set.is_concrete_sample:
                    ...
                    if ref_min is not None:
                        ...
                        if grp_avg < (float(ref_min) - float(min_margin)):
                            result_set.overall_result = 'fail'
    ...
    def action_approve_results(self):
        ...
        # Approve results directly if there's a signature
        self.write({
            'state': 'approved',
        ...
        # If there's a signature, approve and refresh
        return {
            'type': 'ir.actions.client',
            'tag': 'reload',
        }
    ...
    def write(self, vals):
        ...
        # التحقق من قيم الليونة بعد الحفظ
        if 'result_line_ids' in vals:
            for rec in self:
    ...
    def _check_softening_values(self):
        ...
        if len(softening_lines) >= 2:
            # فرز الأسطر حسب كود المعيار للتأكد من الترتيب
...
class LabResultLine(models.Model):
    ...
    def _validate_softening_readings(self, new_value):
        ...
        # البحث عن القراءة الأخرى في نفس مجموعة النتائج ونفس العينة
        ...
        if other_line and other_line.value_numeric:
            ...
            if diff > 1.0:
                ...
                # تحديث حقل التحذير في مجموعة النتائج
                ...
                # مسح التحذير إذا كان الفرق مقبولاً
    ...
    def _compute_sample_identifier(self):
        for line in self:
            if line.criterion_id and line.criterion_id.is_summary_field:
                ...
                # نتيجة محسوبة غير تلخيصية: استخدم نمط "ن-<رقم العينة>"
    ...
    def _compute_compliance(self):
        ...
        for line in self:
            # فحص إذا كانت النتيجة من عينة 7 أيام أو احتياط - تجاهلها للخرسانة
            if line._is_experimental_sample():
                line.is_compliant = True  # تعيين مطابق لتجنب تأثيرها على النتيجة النهائية
                continue
            ...
            if test_type == 'numeric':
                ...
                # فحص متوسط الاختراق حسب نوع القير
                    ...
                    if task and hasattr(task, 'reference_min_limit') and task.reference_min_limit:
                        try:
                            ...
                            # في حالة خطأ، استخدم المنطق العادي
                        ...
                        # إذا لم يوجد معيار مرجعي، استخدم المنطق العادي
                        if line.min_limit is not False and line.max_limit is not False:
                            line.is_compliant = line.min_limit <= line.value_numeric <= line.max_limit
                        elif line.min_limit is not False:
                            line.is_compliant = line.value_numeric >= line.min_limit
                    ...
                    # للفحوصات غير الخرسانة: استخدام المنطق العادي
                    if line.min_limit is not False and line.max_limit is not False:
                        line.is_compliant = line.min_limit <= line.value_numeric <= line.max_limit
                    elif line.min_limit is not False:
                        line.is_compliant = line.value_numeric >= line.min_limit
                ...
                if line.result_set_id.is_concrete_sample and line.criterion_id.code in ['COMP_STRENGTH_CONCRETE', 'AVG_COMP_STRENGTH_CONCRETE']:
                    ...
                    # للفحوصات غير الخرسانة: استخدم المنطق العادي
                    if line.value_numeric == 0:
                        line.is_compliant = True 
                    elif line.min_limit is not False and line.max_limit is not False:
                        line.is_compliant = line.min_limit <= line.value_numeric <= line.max_limit
    ...
    def _is_experimental_sample(self):
        ...
        # إذا وجدت move lines مع age_days = 7 أو reserve، فهي عينة تجريبية
    ...
    def _check_flash_point_compliance(self):
        ...
        if not self.value_numeric:
            return True  # إذا لم تكن هناك قيمة، اعتبرها مطابقة
            
        # تحديد نوع القير من العينة
        if not (self.result_set_id and self.result_set_id.sample_id):
            return True  # إذا لم تكن هناك عينة، استخدم المنطق العادي
            
        sample = self.result_set_id.sample_id
        bitumen_type = getattr(sample, 'bitumen_type', None)
        
        # تحديد الحد الأدنى المطلوب حسب نوع القير
        if bitumen_type == 'قير تسطيح':
            min_required = 302  # قير تسطيح: أكبر من أو يساوي 302
        elif bitumen_type == 'قير أساس':
            min_required = 232  # قير أساس: أكبر من أو يساوي 232 (المستوى الأدنى)
        else:
            # نوع القير غير محدد، استخدم المنطق العادي
            if self.min_limit is not None and self.min_limit is not False:
                return self.value_numeric >= self.min_limit
            return True
            
        # التحقق من القيمة مقابل الحد المطلوب
    ...
    def _check_penetration_compliance(self):
        ...
        if not (self.result_set_id.sample_id and self.result_set_id.sample_id.product_id):
            return True  # إذا لم يكن هناك منتج، استخدم المنطق العادي
        ...
        # تحديد المدايات المقبولة حسب نوع القير
        if 'SURFACE' in product_code or 'تسطيح' in product_name:
            # قير تسطيح - ثلاثة مستويات
            valid_ranges = [
                (18, 60),  # المستوى الأول
                (15, 35),  # المستوى الثاني
                (12, 25),  # المستوى الثالث
            ]
        elif 'BASE' in product_code or 'أساس' in product_name:
            # قير أساس - ثلاثة مستويات
            valid_ranges = [
                (50, 100), # المستوى الأول
                (25, 50),  # المستوى الثاني
                (20, 40),  # المستوى الثالث
            ]
        else:
            # نوع القير غير محدد، استخدم المنطق العادي
        ...
        # التحقق من وقوع القيمة ضمن أي من المدايات المقبولة
        for min_val, max_val in valid_ranges:
            if min_val <= penetration_value <= max_val:
                return True
                
        # إذا لم تقع ضمن أي مدى، الفحص فاشل
        return False
    ...
    def _check_ductility_compliance(self):
        ...
        # تحديد الحدود الدنيا المقبولة حسب نوع القير
        if 'SURFACE' in product_code or 'تسطيح' in product_name:
            # قير تسطيح - أربعة مستويات (حدود دنيا فقط)
            min_limits = [10, 3, 2.5, 1.5]  # النوع الأول، الثاني، الثالث، الرابع
        elif 'BASE' in product_code or 'أساس' in product_name:
            # قير أساس - ثلاثة مستويات (حدود دنيا فقط)
            min_limits = [30, 10, 2]  # المستوى الأول، الثاني، الثالث
        else:
            # نوع القير غير محدد، استخدم المنطق العادي
            if self.min_limit is not False and self.max_limit is not False:
                return self.min_limit <= self.value_numeric <= self.max_limit
        ...
        # التحقق من وقوع القيمة ضمن أي من الحدود الدنيا المقبولة
        for min_limit in min_limits:
            if ductility_value >= min_limit:
                return True
                
        # إذا لم تتجاوز أي حد أدنى، الفحص فاشل
        return False
