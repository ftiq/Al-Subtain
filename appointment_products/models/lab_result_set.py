...
class LabResultSet(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            try:
                ...
                # إسناد test_group_no تلقائياً لغير الخرسانة إذا لم يُحدد
                if not vals.get('test_group_no'):
                    # حدد إن كانت العينة خرسانية من vals أو من سياق السطر أعلاه
                ...
                # إجبار عدد العينات = 1 لكل الفحوصات غير القيرية فقط
                try:
                    is_bitumen = False
                    # اكتشاف عبر كود القالب
                    if vals.get('template_id'):
                        tmpl = self.env['lab.test.template'].browse(vals['template_id'])
                        if tmpl and tmpl.code:
                            is_bitumen = 'BITUMEN' in (tmpl.code or '').upper()
                    # أو عبر نوع/اسم المنتج المرتبط بالعينة
                    if not is_bitumen and vals.get('sample_id'):
                        sample = self.env['lab.sample'].browse(vals['sample_id'])
                        if sample and getattr(sample, 'bitumen_type', False):
                            is_bitumen = True
                        elif sample and sample.product_id:
                            pcode = (sample.product_id.default_code or '').upper()
                            pname = (sample.product_id.name or '')
                            is_bitumen = ('SURFACE' in pcode) or ('BASE' in pcode) or ('تسطيح' in pname) or ('أساس' in pname)
                    if not is_bitumen:
                        vals['number_of_samples'] = 1
                except Exception:
                    # في حال الشك، نترك القيمة كما هي
                    pass
            except Exception:
                pass
    ...
    def action_approve_results(self):
        ...
        # اجعل حالة العينة "مكتملة" بعد اعتماد النتائج
        if self.sample_id and self.sample_id.state != 'completed':
            try:
                self.sample_id.write({'state': 'completed'})
                self.sample_id.message_post(body=_('تم اعتماد النتائج – تحولت حالة العينة إلى "مكتملة"'))
            except Exception:
                # لا تفشل العملية إن تعذّر تحديث حالة العينة لسبب ما
                pass
...
class LabResultLine(models.Model):
    ...
    @api.depends('value_numeric', 'criterion_id.min_value', 'criterion_id.max_value', 
                 ...
                 # إعادة حساب مطابقة فحوص القير عند تغيير مستوى المقارنة أو نوع المنتج
        ...
        for line in self:
            ...
            if test_type == 'numeric':
                ...
                if line.criterion_id.code == 'PENTEST_AVG':
                    ...
                    # يجب أن يطابق مستوى النتيجة المستوى المختار في العينة
                    sample = line.result_set_id.sample_id if line.result_set_id else None
                    chosen = '1'
                    if sample and hasattr(sample, 'bitumen_comparison_level') and sample.bitumen_comparison_level:
                        chosen = sample.bitumen_comparison_level
                    try:
                        chosen_level = int(chosen)
                    except Exception:
                        chosen_level = 1
                    # في حالة فشل الحساب (قيمة -1) يعتبر غير مطابق
    ...
    def _check_penetration_compliance(self):
        ...
        # استخدم مستوى المقارنة المختار على العينة
        sample = self.result_set_id.sample_id
        level_str = getattr(sample, 'bitumen_comparison_level', None) or '1'
        try:
