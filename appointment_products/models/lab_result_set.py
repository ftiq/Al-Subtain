        ...
        for line in summary_criteria:
            try:
                if line.criterion_id.computation_formula:
                    ...
                    _logger.info(
                        "updated_series_data للمعيار التلخيصي %s: %s", 
                        line.criterion_id.code,
                        {k: v for k, v in updated_series_data.items() if 'COMP_STRENGTH' in k or 'LOAD_KN' in k}
                    )
                    

                    summary_context = global_context.copy()
                    for criteria_code, values_list in updated_series_data.items():
                        if values_list: 
...
class LabResultLine(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = 'result_set_id, sample_no, criterion_timer_sequence, sequence, criterion_id, id'
    ...
    @api.depends('sample_no', 'criterion_id.is_summary_field')
