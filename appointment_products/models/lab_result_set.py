...
class LabResultSet(models.Model):
    ...
    _sql_constraints = [
        ...
        ('sample_template_group_non_concrete',
         ...
         'لا يمكن تكرار نفس القالب لنفس العينة ونفس رقم المجموعة (لغير الخرسانة)!'),
        ('positive_samples', 
    ...
    test_group_no = fields.Integer(
        string='رقم المجموعة',
        default=1,
        help='لتجميع تكرارات نفس القالب حسب مجموعة الفحص'
    )
    ...
    field_density_weight = fields.Float(
        string='الوزن',
        help='وزن العينة المستخدم في فحص الكثافة الموقعية (قيمة مرجعية لهذه المجموعة)'
    )
    field_density_volume = fields.Float(
        string='الحجم',
        help='حجم القالب/الحفرة المستخدم في فحص الكثافة الموقعية (قيمة مرجعية لهذه المجموعة)'
    )
    ign_show_second_table = fields.Boolean(
        string='إظهار Table (2)',
        default=False
    )
    ...
    charts_placeholder = fields.Char(string='مخطط النتائج', compute='_compute_empty_placeholder')

    ign_t1_sample_no = fields.Integer(string='العينة', default=1, readonly=True)
    ign_t1_flame_spread = fields.Selection([
        ('lt_150', 'لم يتجاوز (150mm)'),
        ('ge_150', 'تجاوز (150mm)'),
    ], string='مدى انتشار اللهب')
    ign_t1_time_to_150 = fields.Char(string='الوقت اللازم لوصول اللهب إلى ارتفاع (150 ملم)', default='لم يصل')
    ign_t1_ignition_happened = fields.Selection([
        ('yes', 'نعم'),
        ('no', 'لا'),
    ], string='هل حدث الاشتعال', default='no')
    
    ign_t2_sample_no = fields.Integer(string='العينة', default=2, readonly=True)
    ign_t2_flame_spread = fields.Selection([
        ('lt_150', 'لم يتجاوز (150mm)'),
        ('ge_150', 'تجاوز (150mm)'),
    ], string='مدى انتشار اللهب')
    ign_t2_time_to_150 = fields.Char(string='الوقت اللازم لوصول اللهب إلى ارتفاع (150 ملم)', default='لم يصل')
    ...
    ign_t1_surface_time_of_test = fields.Integer(
        string='مدة الاختبار (سطح العينة) [ثانية]', default=30)
    ign_t1_surface_duration = fields.Integer(
        string='فترة الفحص (سطح العينة) [ثانية]', default=60)

    ign_t2_edge_time_of_test = fields.Integer(
        string='مدة الاختبار (حافة العينة) [ثانية]', default=30)
    ign_t2_edge_duration = fields.Integer(
        string='فترة الفحص (حافة العينة) [ثانية]', default=60)

    # مرفقات تقرير الاشتعال (حتى أربع صور)
    ign_image1 = fields.Binary(string='صورة 1 (اشتعال)', attachment=True)
    ign_image2 = fields.Binary(string='صورة 2 (اشتعال)', attachment=True)
    ign_image3 = fields.Binary(string='صورة 3 (اشتعال)', attachment=True)
    ign_image4 = fields.Binary(string='صورة 4 (اشتعال)', attachment=True)
    ...
    def _compute_test_type_flags(self):
        """تحديد نوع الفحص الحالي لإظهار المخططات المناسبة في الواجهة"""
        for record in self:
    ...
    def _compute_is_ignitability_sample(self):
        """تحديد إن كانت العينة من نوع قابلية الاشتعال بناءً على كود نوع العينة للمنتج"""
        for record in self:
            code = ''
            try:
                if record.sample_id \
    ...
    def _compute_is_field_density_sample(self):
        """تحديد ما إذا كانت العينة من نوع FIELD_DENSITY"""
        for record in self:
            code = ''
            try:
                if record.sample_id \
    ...
    is_agg_quality_sample = fields.Boolean(
        ...
        store=True,
        help='يتفعّل عندما يكون نوع العينة AGG_QUALITY'
    )
    ...
    # ملخص جودة الركام (R6/2003)
    agg_selected_class = fields.Selection(
        [('A', 'A'), ('B', 'B'), ('C', 'C'), ('D', 'D')],
        string='فئة المطابقة (R6/2003)',
        default='A',
        tracking=True
    )
    ...
    def _update_dashboard_statistics(self):
        """تحديث إحصاءات اللوحات المخزّنة لضمان سرعة التحميل"""
    ...
    def create(self, vals_list):
            ...
            try:
                if 'reference_general_limit' not in vals:
                    ...
                    if sample_id:
                        ...
                        # تمرير الوزن والحجم لعَيّنات الكثافة الموقعية
                        try:
                            st_code = ''
        ...
        for result_set in result_sets:
            ...
            # إنشاء حدود التدرج الافتراضية لفحص جودة الركام (R6/2003)
            try:
            ...
            # نسخ agg_selected_class من Proctor إلى CBR تلقائياً
            try:
                tmpl_code = (result_set.template_id.code or '').upper() if result_set.template_id else ''
    ...
    def _compute_overall_result(self):
        ...
        for result_set in self:
            ...
            # منطق جودة الركام (R6/2003): يعتمد حصراً على مطابقة كل المناخل ضمن حدود الفئة المختارة
            try:
                tmpl_code = (result_set.template_id.code or '').upper() if result_set.template_id else ''
            except Exception:
                tmpl_code = ''
    ...
    def _compute_overall_result_with_avg(self):
        """عرض النتيجة العامة مع متوسط المقاومة لعَيّنات الخرسانة فقط"""
        for rec in self:
        ...
        for gline in global_lines:
            ...
            try:
                if gline.criterion_id.is_computed_field and gline.criterion_id.computation_formula:
                    _logger.debug(
                        "حساب معيار عام %s - الصيغة: %s - context: %s",
                        code,
                ...
                _logger.info(
                    "نتيجة المعيار العام %s: %s - series_data محدث: %s",
                    code,
                ...
                _logger.error(
                    "خطأ في حساب المعيار العام %s في مجموعة النتائج %s: %s",
                    code,
                    self.name,
                    str(e),
                    exc_info=True
    ...
    def get_agg_selected_class_from_sample(self):
        ...
        # البحث عن Proctor أو Aggregate Quality في نفس العينة
    ...
    def _agg_quality_eval_status(self):
        """تقييم حالة مطابقة جودة الركام (R6/2003) لهذه المجموعة.
        ترجع 'pass' أو 'fail' أو 'pending'.
        المنطق: يجب أن تكون كل المناخل المطلوبة (ذات حدود فعّالة غير صفرية)
        ضمن [effective_min, effective_max]. إذا نقصت قيمة لازمة لأي منخل مطلوب → pending.
        إذا خرج أي منخل عن المدى → fail. وإلا → pass.
        """
        self.ensure_one()
    ...
    def _compute_sample_count(self):
        for record in self:
          ...
          #يراجع نسخة قديمة (غير مستخدمة) تُركت للرجوع فقط
    def _compute_overall_conformity_legacy(self):
        """نسخة قديمة (غير مستخدمة) تُركت للرجوع فقط"""
        pass
    ...
    bitumen_comparison_level = fields.Selection(
        ...
        string='مستوى المقارنة (قير) - المجموعة',
        help='تطبيق مستوى المقارنة على كامل مجموعة النتائج لهذه المجموعة فقط. في حال عدم التحديد سيتم استخدام مستوى العينة.'
    )
    ...
    def write(self, vals):
        ...
        # مزامنة مستوى المقارنة على كامل المجموعة (فحوص القير) لنفس العينة ورقم المجموعة
...
class LabResultLine(models.Model):
    ...
    @api.model_create_multi
    def create(self, vals_list):
        """إنشاء أسطر النتائج دون أي حقول غير موجودة على الموديل"""
