...
class LabResultSet(models.Model):
    ...
    def _compute_overall_result(self):
        ...
        for result_set in self:
            ...
            # إذا لم يتم ملء أي حقول إدخال، النتيجة معلقة
            if not filled_input_lines and input_lines:
                result_set.overall_result = 'pending'
                continue
            
            # إذا لم يتم ملء جميع الحقول المطلوبة، النتيجة معلقة
            if len(filled_input_lines) < len(input_lines):
                result_set.overall_result = 'pending'
                continue

            # الآن فقط احسب النتيجة الفعلية
            ...
            # قاعدة خاصة للخرسانة: إذا كان متوسط قوة الضغط أقل من المعيار الأدنى بـ 3 → فشل
            try:
                if result_set.is_concrete_sample:
                    ...
                    if ref_min is not None:
                        ...
                        if grp_avg < (float(ref_min) - 3):
                            result_set.overall_result = 'fail'
                            _logger.info(f"Concrete group {result_set.name} marked as FAIL: avg {grp_avg} < (ref_min {ref_min} - 3)")
            except Exception:
                # لا نكسر الحساب في حال خطأ
                pass
    ...
    def action_approve_results(self):
        ...
        for record in self:
            ...
            record.message_post(
                body="✅ <b>تم اعتماد النتائج نهائياً</b>",
                message_type='notification'
            )
            ...
            if record.testers_ids:
                ...
                for tester in record.testers_ids:
                    final_text = dict(record._fields['overall_result'].selection).get(record.overall_result, record.overall_result)
                    record.message_post(
                        body=(
                            "✅ <b>تم اعتماد نتائج الفحص المكلف به</b>"
                            "<div style='margin-top:6px'>"
                            f"🧪 <b>العينة:</b> {record.sample_id.display_name}<br/>"
                            f"📋 <b>قالب الفحص:</b> {record.template_id.display_name}<br/>"
                            f"🏁 <b>النتيجة النهائية:</b> {final_text}<br/>"
                            f"📈 <b>نسبة الإنجاز:</b> {record.progress_percentage:.1%}"
                            "</div>"
                            f"<div style='margin-top:8px;'><a href='/web#id={record.id}&model=lab.result.set&view_type=form'>🔎 عرض التقرير</a></div>"
                        ),
                        message_type='notification',
                        partner_ids=[tester.partner_id.id],
                        subtype_id=self.env.ref('mail.mt_note').id
        ...
        sample._compute_all_results_approved()  
        
        if sample.all_results_approved and sample.state == 'testing':
            sample.state = 'completed'
            sample.message_post(body=_('تم إكمال العينة تلقائياً بعد اعتماد جميع النتائج'))
