...
class LabSample(models.Model):
    ...
    def create(self, vals):
        ...
        # تعيين وقت التوقيع عند إنشاء السجل إذا تم تمرير التوقيع
    ...
    # --- Signature clear helpers on sample ---
    ...
    def write(self, vals):
        ...
        # منع تعديل/حذف التوقيع بعد مرور 24 ساعة
        ...
        # تحديث وقت التوقيع عند إضافة/تعديل التوقيع
        for rec in self:
            updates = {}
            if 'exec_signature' in vals:
    ...
    # === حقول ودوال المجموعات المتعاقبة للخرسانة ===
    ...
    def _compute_consecutive_groups_data(self):
        ...
        for sample in self:
            ...
            # جلب مجموعات 28 يوم مرتبة حسب رقم المجموعة
            ...
            if consecutive_data:
                ...
                if ref_general or ref_min:
                    ...
                    html += '<strong>المراجع:</strong> '
                    if ref_general:
                        html += f'المعيار العام: {ref_general:.2f} N/mm² '
                    if ref_min:
                        html += f'| المعيار الأدنى: {ref_min:.2f} N/mm²'
                    html += '</div>'
                
                html += '</div>'
                
                sample.consecutive_groups_data = html
            else:
                sample.consecutive_groups_data = '<p class="text-muted">لا توجد بيانات كافية لحساب المجموعات المتعاقبة</p>'
