...
class LabSample(models.Model):
    ...
    activity_state = fields.Selection([
        ...
        ('today', _('Today')),
        ('planned', _('Planned'))
    ], string=_('Activity State'), compute='_compute_activity_state', sortable=False)
    ...
    def action_complete(self):
        ...
        if uncompleted_sets:
            uncompleted_names = ', '.join(uncompleted_sets.mapped('name'))
            raise UserError(_(
                'لا يمكن إكمال العينة حتى يتم إكمال جميع مجموعات النتائج!\n'
                'المجموعات غير المكتملة: %s\n\n'
                'يرجى إكمال ومراجعة جميع النتائج أولاً قبل إكمال العينة.'
            ) % uncompleted_names)
        
        unapproved_sets = self.result_set_ids.filtered(lambda rs: rs.state != 'approved')
        if unapproved_sets:
            unapproved_names = ', '.join(unapproved_sets.mapped('name'))
            raise UserError(_(
