...
class LabSample(models.Model):
    ...
    # --- Signatures (independent on sample) ---
        ...
        for sample in self:
            ...
            for rs in all_result_sets:
                if sample._is_result_set_experimental(rs):
                    continue  # تجاهل العينات التجريبية
                non_experimental_sets.append(rs)
            

            if not non_experimental_sets:
                sample.all_results_approved = True  # لا توجد عينات غير تجريبية
            else:
    ...
    def _compute_is_concrete_sample(self):
        ...
        concrete_codes = ['CONCRETE_BUILDINGS', 'CONCRETE_FOUNDATIONS', 'CONCRETE_BRIDGES']
        for sample in self:
    ...
    def _compute_consecutive_groups_data(self):
        ...
        for sample in self:
            ...
            # حساب المتوسطات المتعاقبة (كل 4 مجموعات)
            consecutive_data = []
            window_size = 4
            
            # جلب المراجع للمقارنة
            ref_general = 0
            ref_min = 0
            general_margin = 3.0
            min_margin = 3.0
    ...
    def _compute_bitumen_results_summary(self):
        ...
        for record in self:
            ...
            # جلب جميع نتائج الفحص المرتبطة بالعينة
            ...
            for result_set in result_sets:
                ...
                # عرض جميع المعايير لهذا القالب (مرتبة حسب sequence) مع فلترة نتيجة فحص الليونة
                for line in result_set.result_line_ids.sorted('sequence'):
                    if line.criterion_id and line.is_filled and 'نتيجة فحص الليونة' not in line.criterion_id.name:
                        ...
                        # إضافة صف للمعيار (بدون عمود النتيجة)
                        html += f"""
                            <tr>
                ...
                # إضافة جداول المستويات للفحوصات المهمة
    ...
    def _get_penetration_level(self, value):
        ...
        if self.bitumen_type == 'قير تسطيح':
            if 200 <= value <= 300:
                return "النوع الأول", "200-300"
            elif 100 <= value <= 200:
                return "النوع الثاني", "100-200"
            elif 50 <= value <= 100:
                return "النوع الثالث", "50-100"
            elif 25 <= value <= 50:
                return "النوع الرابع", "25-50"
            else:
    ...
    def _get_softening_level(self, value):
        ...
        if self.bitumen_type == 'قير تسطيح':
            if 45 <= value <= 52:
                return "النوع الأول", "45-52°م"
            elif 52 <= value <= 58:
                return "النوع الثاني", "52-58°م"
            elif 57 <= value <= 66:
                return "النوع الثالث", "57-66°م"
            elif 63 <= value <= 75:
                return "النوع الرابع", "63-75°م"
            else:
                return "خارج المواصفات", f"القيمة {value}°م خارج النطاقات المقبولة"
                
        elif self.bitumen_type == 'قير أساس':
            if 45 <= value <= 52:
                return "المستوى الأول", "45-52°م"
            elif 52 <= value <= 58:
                return "المستوى الثاني", "52-58°م"
            elif 57 <= value <= 66:
                return "المستوى الثالث", "57-66°م"
            else:
                return "خارج المواصفات", f"القيمة {value}°م خارج النطاقات المقبولة"
    ...
    def _get_all_penetration_levels(self):
        ...
        if self.bitumen_type == 'قير تسطيح':
            return [
                ("المستوى الأول", "18-60", "0.1 مم"),
                ("المستوى الثاني", "15-35", "0.1 مم"),
                ("المستوى الثالث", "12-25", "0.1 مم"),
            ]
        elif self.bitumen_type == 'قير أساس':
            return [
    ...
    def _get_all_softening_levels(self):
        ...
        if self.bitumen_type == 'قير تسطيح':
            return [
                ("النوع الأول", "45-52", "°م"),
                ("النوع الثاني", "52-58", "°م"),
                ("النوع الثالث", "57-66", "°م"),
                ("النوع الرابع", "63-75", "°م"),
            ]
        elif self.bitumen_type == 'قير أساس':
            return [
                ("المستوى الأول", "45-52", "°م"),
                ("المستوى الثاني", "52-58", "°م"),
                ("المستوى الثالث", "57-66", "°م"),
            ]
        return []
    ...
    def _generate_levels_table(self, test_type, current_value=None):
        ...
        for level_name, range_value, unit in levels:
            is_applied = level_name == applied_level
            row_style = "background-color: #fff3cd;" if is_applied else ""
            check_mark = "✅" if is_applied else ""
