...
class LabSampleExcelExportWizard(models.TransientModel):
    ...
    # حقل مساعد لتبسيط الدومين في الواجهة دون استخدام مسار dot
    ...
    def action_open_spreadsheet(self):
        ...
        # أنشئ Spreadsheet مؤقت عبر spreadsheet.mixin فقط للمعاينة (بدون حفظ كمستند)
        # أنشئ سجل جلسة Spreadsheet مؤقتة بوراثة spreadsheet.mixin
        session = self.env['lab.spreadsheet.session'].create({
            'name': f"Spreadsheet - {self.sample_id.name}",
        })
        # خزّن بيانات فارغة كبداية
        ...
        # حقن الجداول في جلسة Spreadsheet
        if False:
            try:
                # اجمع كل قوائم Odoo الموجودة في القالب والتي تعمل على نموذج lab.result.line
                ...
                # أضف الفلتر العام إن لم يكن موجوداً، وإن وجد عدّل الربط فقط
                if mapping:
                    add_cmd = {
                ...
                # اضبط قيمة الفلتر العام إلى العينة الحالية
            ...
            # حقن عدة جداول منفصلة: جدول لكل مجموعة نتائج (result_set) لسهولة التتبع
            try:
                ...
                for rs in result_sets:
                    ...
                    lines_num = max(lines_count + 5, 20)  # نحتفظ بهامش صفوف إضافي
                    ...
                    try:
                        # استخدم سجل الجلسة لبث الأوامر
                        session._dispatch_commands([title_cmd, insert_cmd])
                    except Exception:
                        pass

                    # فاصل سطرين قبل الجدول التالي
    ...
    def action_export_via_spreadsheet(self):
        ...
        # التصدير يتم من واجهة JS عادةً، لكن سننشئ تنزيل مرفق الـ excel_export إن كان مجمداً
        ...
        # محاكاة تنزيل: إن كان لدينا excel_export (من freeze) نستخدمه، وإلا نعرض فتح المحرر ليقوم المستخدم بالتصدير
        if doc.excel_export:
    ...
    def action_export_excel(self):
        ...
        # مسار قديم للتصدير من Spreadsheet – مُعطل الآن
        if False and self.current_spreadsheet_attachment_id:
            # تصدير XLSX مباشرة من محرر Spreadsheet لاحقاً (مكان الربط المستقبلي)
            pass

        # المسار القياسي القديم
    ...
    def _add_results_summary(self, worksheet, start_row):
        ...
        try:
            ...
            general_margin = 3.0
        # قراءة هامش المعيار الأدنى من الإعدادات لاستخدامه في تعليم المجموعات الفردية
        try:
        ...
        try:
            ...
            if not is_bridges and ref_general is not None and len(group_avgs) >= 4:
                ...
                for i in range(0, len(group_avgs) - window + 1):
                    ...
                    # وفق المنطق الجديد: النجاح عندما المتوسط >= (المعيار العام + الهامش)
                    # والفشل عندما المتوسط < (المعيار العام + الهامش)
    ...
    def _add_concrete_compression_report(self, ws, start_row):
        ...
        if not is_bridges:
            # عرض المعيار العام والمعيار الأدنى والعتبة (المعيار العام + الهامش)
            try:
                general_margin = float(self.env['ir.config_parameter'].sudo().get_param('appointment_products.general_limit_margin', default='3'))
            except Exception:
                general_margin = 3.0
    ...
    def _add_group_rolling_averages(self, worksheet, start_row, reference_general_limit=None, window_size=4):
        ...
        - إذا توفّر reference_general_limit: ضع نجمة * عندما يكون متوسط النافذة أقل من (المعيار العام + 3).
