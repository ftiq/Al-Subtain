    ...
    def _apply_template_lines(self):
        ...
        try:
            if self.sample_id and self.sample_id.product_id and getattr(self.sample_id.product_id.product_tmpl_id, 'sample_type_id', False):
                ...
                is_ignitability = (st_code == 'IGNITABILITY')
                is_field_density = (st_code == 'FIELD_DENSITY')

            if self.template_id and self.template_id.line_ids:
                if not is_ignitability:
                    is_ignitability = any('IGN' in (l.test_template_id.code or '').upper() for l in self.template_id.line_ids)
                if not is_field_density:
                    is_field_density = any('FIELD_DENSITY' in (l.test_template_id.code or '').upper() for l in self.template_id.line_ids)
        except Exception:
            is_ignitability = False
            is_field_density = False
...
class LabTestFlowLine(models.Model):
    ...
    def _launch_result_set(self):
        ...
        field_serial = False
        try:
            sample = self.flow_id.sample_id
            picking = sample.task_id.stock_receipt_id if sample and sample.task_id else False
            if picking and sample.product_id and self.group_no:
                ml = self.env['stock.move.line'].search([
                    ('picking_id', '=', picking.id),
                    ('product_id', '=', sample.product_id.id),
                    ('group_no', '=', int(self.group_no or 1)),
                ], limit=1)
                if ml:
                    lab_code = ml.field_code or False
                    field_serial = ml.field_serial or False
        except Exception:
            pass
