...
class LabTestTemplate(models.Model):
    ...
    @api.depends('standard_reference')
    def _compute_standard_code(self):
        for rec in self:
            rec.standard_code = rec.standard_reference

    def _inverse_standard_code(self):
        for rec in self:
            rec.standard_reference = rec.standard_code
...
class LabTestCriterion(models.Model):
    ...
    is_time_based = fields.Boolean(
        string='معيار وقتي',
        default=False,
        help='هل هذا المعيار مرتبط بمؤقت زمني؟'
    )
    
    time_duration = fields.Float(
        string='المدة الزمنية',
        help='المدة الزمنية المطلوبة قبل إدخال القيمة'
    )
    ...
    time_unit = fields.Selection([
        ...
        ('hours', 'ساعات'),
        ('days', 'أيام')
    ], string='وحدة الوقت', default='minutes')
    
    timer_dependent_criteria = fields.Many2many(
        'lab.test.criterion',
        'timer_dependent_criteria_rel',
        'timer_criterion_id',
        'dependent_criterion_id',
        string='المعايير التابعة للمؤقت',
        help='المعايير التي يجب أن تنتظر انتهاء هذا المؤقت قبل إدخال قيمها',
        domain=[('is_time_based', '=', False)]
    )
