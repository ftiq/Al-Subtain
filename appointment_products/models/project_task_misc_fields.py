...
class ProjectTaskMiscFields(models.Model):
    ...
    move_ids_display = fields.Char(
        string='Moves Summary', 
        readonly=True, 
        copy=False
    )
    ...
    appointment_datetime = fields.Datetime(
        string='موعد الحجز', 
        related='sale_order_id.appointment_datetime', 
        store=True,
        index=True
    )
    ...
    total_duration_hours = fields.Float(
        string='الوقت الكلي (ساعة)',
        compute='_compute_total_duration_hours',
        store=True,
        help='فرق وقت الانتهاء عن وقت البدء بالساعة.'
    )
    ...
    def _compute_total_duration_hours(self):
        ...
        for task in self:
            try:
                if task.planned_date_begin and task.date_deadline and task.date_deadline >= task.planned_date_begin:
                    delta = task.date_deadline - task.planned_date_begin
                    task.total_duration_hours = round(delta.total_seconds() / 3600.0, 2)
                else:
                    task.total_duration_hours = 0.0
            except Exception as e:
                _logger.error("خطأ في حساب مدة المهمة %s: %s", task.name, str(e))
                task.total_duration_hours = 0.0

    stock_done = fields.Boolean(
        string='انتهت المهمة المخزنية', 
        default=False, 
        tracking=True,
        index=True
    )
    ...
    reference_min_limit = fields.Float(
        ...
        help='الحد الأدنى المرجعي (تُقبل الأرقام الكسرية) للاستخدام في المطابقة.'
    )
    
    @api.onchange('reference_general_limit')
    def _onchange_reference_general_limit(self):
        """عند تغيير المعيار العام، يتم ملء المعيار الأدنى بنفس القيمة تلقائياً."""
        for rec in self:
            rec.reference_min_limit = rec.reference_general_limit
    stock_done_date = fields.Datetime(
        string='تاريخ إنهاء المخزون', 
        tracking=True
    )

    def create(self, vals):
        """Ensure reference_min_limit mirrors reference_general_limit when not explicitly given.
        يدعم إنشاء سجل واحد أو عدة سجلات دفعة واحدة.
        """

        if isinstance(vals, list):
            for v in vals:
                if 'reference_general_limit' in v and 'reference_min_limit' not in v:
                    v['reference_min_limit'] = v.get('reference_general_limit')
        else:
            v = vals
            if 'reference_general_limit' in v and 'reference_min_limit' not in v:
                v['reference_min_limit'] = v.get('reference_general_limit')
    ...
    def write(self, vals):
        ...
        try:
            ...
            if 'reference_general_limit' in vals and 'reference_min_limit' not in vals:
                vals['reference_min_limit'] = vals.get('reference_general_limit')
            ...
            if 'total_samples_count' in vals and not self.env.context.get('bypass_samples_update'):
                for task in self:
                    old_count = old_values.get(task.id, 0)
                    if task.total_samples_count != old_count:
                        _logger.info("تغيير عدد العينات للمهمة %s من %s إلى %s", 
                                   task.name, old_count, task.total_samples_count)
                        task.with_context(bypass_samples_update=True)._update_samples_related_data()
            
            return result
            
        except Exception as e:
            _logger.error("خطأ في تحديث المهمة: %s", str(e))
            raise UserError(_("فشل في تحديث المهمة: %s") % str(e))
