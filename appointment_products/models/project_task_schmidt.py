...
class ProjectTask(models.Model):
    ...
    def _compute_show_schmidt_ui(self):
        for task in self:
            # Show the tab only if the task contains the specified product "Sample - Schmidt Hammer Test"
            show = False
            try:
    ...
    def action_generate_schmidt_sample_and_results(self):
        ...
        self.ensure_one()
        task = self
        # Check if Schmidt Hammer Test sample type is available in the task forms
        ...
        # Get the product (product.product) for the Schmidt Hammer Test sample from the template record
        sample_tmpl = self.env.ref('appointment_products.product_schmidt_hammer_sample', raise_if_not_found=False)
        if not sample_tmpl:
            raise ValidationError(_('Product "Schmidt Hammer Test" sample not found.'))

        # Prevent duplicates: If a Schmidt Hammer Test sample already exists for this task, prevent creation
        ...
        # احصل على المنتج (product.product) الخاص بعينة شميدت من سجل القالب
        ...
        # Create the results set for the SCHMIDT_HAMMER_TEST template
        ...
        # Fill coefficients A and B
        line_a = rs.result_line_ids.filtered(lambda l: l.criterion_id.code == 'SCH_A')[:1]
        line_b = rs.result_line_ids.filtered(lambda l: l.criterion_id.code == 'SCH_B')[:1]
        if line_a:
            line_a.value_numeric = float(task.sch_a or 0.0)
        if line_b:
            line_b.value_numeric = float(task.sch_b or 0.0)

        # Fill R.N values for the points
        for p in points:
        ...
        # Calculate the calculated criteria
        try:
            # Some systems have this method, otherwise we use the direct calculation
            if hasattr(rs, 'action_calculate_results'):
                rs.action_calculate_results()
            else:
...
class ProjectTaskSchmidtPoint(models.Model):
    ...
    @api.depends('rn_avg', 'task_id.sch_a', 'task_id.sch_b')
    def _compute_comp_est(self):
        for rec in self:
            try:
                a = float(rec.task_id.sch_a or 0.0)
                b = float(rec.task_id.sch_b or 0.0)
