...
class ProjectTaskSignatures(models.Model):
    ...
    def _compute_datetime_display(self):
        """حساب عرض التاريخ/الوقت بنفس تنسيق الواجهة"""
        for rec in self:
            ...
            if rec.planned_date_begin:
                ...
                user_datetime = rec.planned_date_begin.replace(tzinfo=pytz.UTC).astimezone(pytz.timezone(user_tz))
            ...
            if rec.date_deadline:
                ...
                user_datetime = rec.date_deadline.replace(tzinfo=pytz.UTC).astimezone(pytz.timezone(user_tz))
