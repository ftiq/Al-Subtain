...
class ProjectTaskStockReceipt(models.Model):
    ...
    # الكثافة الموقعية: إظهار حقول الوزن والحجم على مستوى المهمة لاستخدامها كقيم افتراضية للعينات
    is_field_density_task = fields.Boolean(
        string='Field Density Task',
        compute='_compute_is_field_density_task',
        store=False,
        help='True إذا كانت المهمة تحتوي على منتجات من نوع عينة FIELD_DENSITY'
    )

    field_density_weight = fields.Float(
        string='الوزن',
        help='وزن العينة المستخدم في فحص الكثافة الموقعية (قيمة مرجعية تُطبَّق افتراضياً على عينات المهمة)'
    )

    field_density_volume = fields.Float(
        string='الحجم',
        help='حجم القالب/الحفرة المستخدم في فحص الكثافة الموقعية (قيمة مرجعية تُطبَّق افتراضياً على عينات المهمة)'
    )
    ...
    @api.depends('form_line_ids.product_id.product_tmpl_id.sample_type_id.code', 'form_line_ids.sample_type_id.code')
    def _compute_is_field_density_task(self):
        """تحديد ما إذا كانت المهمة تحتوي على منتج من نوع FIELD_DENSITY"""
        for task in self:
            found = False
            try:
                for line in task.form_line_ids:
                    st_code = ''
                    if line.sample_type_id and line.sample_type_id.code:
                        st_code = (line.sample_type_id.code or '').upper()
                    elif line.product_id and line.product_id.product_tmpl_id and line.product_id.product_tmpl_id.sample_type_id:
                        st_code = (line.product_id.product_tmpl_id.sample_type_id.code or '').upper()
                    if 'FIELD_DENSITY' in st_code:
                        found = True
                        break
            except Exception:
                found = False
            task.is_field_density_task = found
