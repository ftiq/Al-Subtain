...
class StockPickingQualityLabIntegration(models.Model):
    ...
    def _create_automatic_quality_checks(self):
        ...
        # اربط المهمة الحالية بهذا الانتقاء إن وجدت
        task = self.env['project.task'].search([
        ...
        for move_line in self.move_line_ids:
            ...
            # جمع بيانات مجموعات الخرسانة (يشمل الكور والأنواع الأخرى)
            if (is_concrete_non_core or is_concrete_core) and (move_line.group_no or move_line.age_days or move_line.cube_count):
                if product.id not in concrete_products_data:
                    ...
                    # ثبّت علم الكور إن كان المنتج كوراً
                    if is_concrete_core:
                ...
                # لا ننشئ العينات هنا للخرسانة - سيتم إنشاؤها بعد انتهاء الحلقة
                continue

            # غير الخرسانة: معالجة تدفقات القير وباقي الأنواع

            # اختيار خطة الفحص الفعلية حسب النوع الفرعي
        ...
        # إنشاء عينات الخرسانة ومجموعات النتائج (يشمل الكور)
        for product_id, concrete_data in concrete_products_data.items():
            ...
            # اختيار القالب بحسب نوع الخرسانة (كور/غير كور)
            if is_core:
                ...
                try:
                    ...
                    for group_key, group_data in groups.items():
                        ...
                        result_set_vals = {
                            ...
                            # للكور نخلي الحقول الزمنية فارغة، ولغيره نملؤها
