    ...
    def _create_automatic_quality_checks(self):
        ...
        for move_line in self.move_line_ids:
            ...
            is_concrete = (product.product_tmpl_id.sample_type_id and 
                          product.product_tmpl_id.sample_type_id.code in ['CONCRETE_BUILDINGS', 'CONCRETE_FOUNDATIONS', 'CONCRETE_BRIDGES', 'CONCRETE_CORE'])
                ...
                try:
                    ...
                    for group_key, group_data in groups.items():
                        ...
                        result_set_vals = {
                            ...
                            # ملء تاريخ الصب تلقائياً عند اشعار المختبر
                            'casting_date': fields.Date.today(),
                            # حساب العمر المطلوب للفحص بناءً على عمر الخرسانة
                            'required_age_days': group_data['age_days'] or '28',
                        }
                    ...
                    # إضافة قالب "متوسطات مجموعات محددة للخرسانة"
        ...
        for move_line in self.move_line_ids:
                    ...
                    for lab_code_info in lab_codes:
                        ...
                        if not existing_sample:
                            ...
                            try:
                                ...
                                self.message_post(
                                    body=f"🧪 تم إنشاء عينة مختبرية {sample.name}<br/>"
                                        f"📦 المنتج: {product.name}<br/>"
                                        f"🏷️ رمز مختبري: {lab_code}<br/>"
                                        f"📅 تاريخ الصب: {fields.Date.today()}{plan_info}"
                                )
                            except Exception as e:
                                sample.sudo().unlink()
            ...
            if product.product_tmpl_id.sample_type_id and product.product_tmpl_id.sample_type_id.code in ['CONCRETE_BUILDINGS', 'CONCRETE_FOUNDATIONS', 'CONCRETE_BRIDGES', 'CONCRETE_CORE']:
                continue  
