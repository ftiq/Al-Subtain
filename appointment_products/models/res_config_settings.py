...
class ResConfigSettings(models.TransientModel):
    ...
    concrete_general_limit_margin = fields.Float(
        string='هامش المعيار العام (+)',
        help='قيمة الهامش المضافة إلى المعيار العام لتحديد عتبة النجاح لمتوسطات المجموعات المتتابعة (افتراضياً 3).',
        default=3.0,
        config_parameter='appointment_products.general_limit_margin'
    )

    concrete_min_limit_margin = fields.Float(
        string='هامش المعيار الأدنى (-)',
        help='قيمة الهامش المطروحة من المعيار الأدنى لتحديد فشل متوسط المجموعة الفردية (افتراضياً 3).',
        default=3.0,
        config_parameter='appointment_products.min_limit_margin'
    )
        ...
        try:
            ...
            for config in self.sample_product_configs:
                if config.sample_product_id:
                    ...
                    self.env['ir.config_parameter'].sudo().set_param(param_key_price, str(config.fixed_price or 0.0))
