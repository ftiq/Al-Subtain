...
class SampleProductConfig(models.TransientModel):
    ...
    @api.constrains('sample_product_id', 'config_settings_id')
    def _check_duplicate_sample(self):
        """تمنع تكرار نفس منتج العينة داخل نفس معالج الإعدادات."""
        for rec in self:
            if not rec.sample_product_id or not rec.config_settings_id:
                continue
            duplicates = self.search([
                ('id', '!=', rec.id),
                ('config_settings_id', '=', rec.config_settings_id.id),
                ('sample_product_id', '=', rec.sample_product_id.id),
            ], limit=1)
            if duplicates:
                raise ValidationError('لا يمكن تكرار منتج العينات في نفس الإعدادات!')
...
class ResConfigSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        for config in self.sample_product_configs:
            if config.sample_product_id and config.samples_per_unit:
                param_key = f'appointment_products.samples_per_unit_{config.sample_product_id.id}'
                self.env['ir.config_parameter'].sudo().set_param(
                    param_key,
                    str(config.samples_per_unit)
                ) 