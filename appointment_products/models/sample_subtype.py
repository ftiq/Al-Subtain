...
class LabSampleSubtype(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if masonry_type and vals.get('sample_type_id') and vals['sample_type_id'] != masonry_type.id:
                raise UserError(_('النوع الفرعي يجب أن يكون مرتبطاً بنوع طابوق فقط.'))
    ...
    def write(self, vals):
        ...
        if self.env.uid != SUPERUSER_ID:
            ...
            if masonry_type and any(rec.sample_type_id != masonry_type for rec in self):
                raise UserError(_('لا يمكن تعديل الحقول المسموحة إلا للأنواع الفرعية الخاصة بالطابوق.'))

        if 'sample_type_id' in vals and masonry_type and vals['sample_type_id'] != masonry_type.id:
            raise UserError(_('النوع الفرعي يجب أن يكون مرتبطاً بنوع طابوق فقط.'))
