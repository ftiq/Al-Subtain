...
class StockMoveLineFieldCode(models.Model):
    ...
    def _onchange_age_days_auto_cube_count(self):
        ...
        for rec in self:
            ...
            if not rec._is_lab_sample_line():
                continue
            if rec.age_days == '28':
                rec.cube_count = 3
    ...
    ign_person_name = fields.Char(
        string='الاسم',
        help='يظهر فقط لعينة قابلية الاشتعال (Ignitability) لكتابة الاسم المطلوب في التقرير.',
        copy=False,
    )

    ign_site_location = fields.Char(
        string='الموقع',
        help='يظهر فقط لعينة قابلية الاشتعال (Ignitability) لكتابة موقع العينة/العنصر.',
        copy=False,
    )
    ...
    is_ignitability_sample = fields.Boolean(
        ...
        store=False,
        help='قيمة مشتقة لتحديد ما إذا كان هذا السطر يخص منتج عينات نوعه قابليه الاشتعال (IGNITABILITY).'
    )

    # هل المنتج في هذا السطر هو منتج عينة؟ (من product.template.is_sample_product)
    is_sample_product_line = fields.Boolean(
        string='منتج عينة',
        compute='_compute_is_sample_product_line',
        store=False,
        help='True إذا كان المنتج مرتبطاً بقالب منتج تم وسمه كمنتج عينة (product.template.is_sample_product)'
    )
    ...
    @api.depends(
        ...
        'product_id.product_tmpl_id.sample_type_id',
        'product_id.product_tmpl_id.sample_type_id.code'
    )
    def _compute_is_ignitability_sample(self):
        """تحديد إذا كان منتج هذا السطر ينتمي إلى نوع عينة "قابلية الاشتعال".
        يعتمد على رمز نوع العينة في المنتج (sample_type.code == 'IGNITABILITY').
        """
        for rec in self:
            try:
                code = ((rec.product_id and rec.product_id.product_tmpl_id and
                         rec.product_id.product_tmpl_id.sample_type_id and
                         rec.product_id.product_tmpl_id.sample_type_id.code) or '').upper()
            except Exception:
                code = ''
            rec.is_ignitability_sample = (code == 'IGNITABILITY')

    @api.depends('product_id', 'product_id.product_tmpl_id', 'product_id.product_tmpl_id.is_sample_product')
    def _compute_is_sample_product_line(self):
        for rec in self:
            try:
                rec.is_sample_product_line = bool(
                    rec.product_id and rec.product_id.product_tmpl_id and
                    getattr(rec.product_id.product_tmpl_id, 'is_sample_product', False)
                )
            except Exception:
                rec.is_sample_product_line = False

    def _is_lab_sample_line(self):
        """تحديد ما إذا كان هذا السطر يخص سياق العينات المخبرية فقط.
        يمنع التأثير على جميع حركات المخزون العامة.
        المعايير:
        - المنتج مميز كمنتج عينات، أو له نوع عينة محدد.
        - أو حركة المخزون مرتبطة بنوع عينة خرسانية/مباني.
        """
        self.ensure_one()
        try:
            prod_tmpl = self.product_id.product_tmpl_id if self.product_id else False
            if prod_tmpl and (getattr(prod_tmpl, 'is_sample_product', False) or
                              (prod_tmpl.sample_type_id and prod_tmpl.sample_type_id.code)):
                return True
            picking = self.picking_id
            if picking and (
                picking.concrete_sample_type_id or
                getattr(picking, 'is_concrete_sample', False) or
                getattr(picking, 'is_masonry_sample', False)
            ):
                return True
        except Exception:
            pass
    ...
    @api.model
    def _is_lab_sample_vals(self, vals):
        """تحقق من كون القيم تخص سياق عينات مخبرية للحد من التأثير على الحركات العامة."""
        try:
            product_id = vals.get('product_id')
            move_id = vals.get('move_id')
            picking_id = vals.get('picking_id')
            if not product_id and move_id:
                mv = self.env['stock.move'].browse(move_id)
                if mv.exists():
                    product_id = mv.product_id.id
                    picking_id = picking_id or (mv.picking_id.id if mv.picking_id else False)
            if product_id:
                prod = self.env['product.product'].browse(product_id)
                if prod.exists():
                    tmpl = prod.product_tmpl_id
                    if getattr(tmpl, 'is_sample_product', False):
                        return True
                    st = getattr(tmpl, 'sample_type_id', False)
                    if st and st.code:
                        return True
            if picking_id:
                pk = self.env['stock.picking'].browse(picking_id)
                if pk.exists():
                    if pk.concrete_sample_type_id:
                        return True
                    if getattr(pk, 'is_concrete_sample', False) or getattr(pk, 'is_masonry_sample', False):
                        return True
        except Exception:
            return False
        return False
    ...
    def _ensure_required_fields(self, vals):
        ...
        if not vals.get('qty_done'):
            if self._is_lab_sample_vals(vals):
                vals['qty_done'] = 1.0
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if self._is_lab_sample_vals(vals) and vals.get('age_days') and not vals.get('cube_count'):
                if vals['age_days'] == '28':
                    vals['cube_count'] = 3
    ...
    def write(self, vals):
        ...
        if 'cube_count' in vals or 'seven_day_cubes' in vals or 'twenty_eight_day_cubes' in vals:
            for record in self:
                ...
                if not record._is_lab_sample_line():
                    continue
