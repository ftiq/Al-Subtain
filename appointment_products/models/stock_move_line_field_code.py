...
class StockMoveLineFieldCode(models.Model):
    ...
    def unlink(self):
        """منع حذف خطوط العينات بعد التوقيع الإلكتروني وحذف السطر المقابل من حركة المخزون"""
        import logging
        ...
        # جمع معلومات السطور للحذف من stock.picking المقابل
        lines_to_delete_from_picking = []
        for record in self:
            _logger.info("Processing deletion of move_line %s (group_no=%s, field_code=%s)", 
                        record.id, record.group_no, record.field_code)
            
            # البحث عن المهمة المرتبطة بهذا السطر
            task = self.env['project.task'].search([('move_line_ids', 'in', record.id)], limit=1)
            if task and task.stock_receipt_id:
                picking = task.stock_receipt_id
                _logger.info("Found task %s with picking %s", task.name, picking.name)
                
                # البحث عن السطر المقابل في stock.picking بناءً على المعايير المطابقة
                picking_lines = picking.move_line_ids.filtered(
                    lambda line: (
                        line.id != record.id and  # تجنب نفس السطر
                        line.product_id == record.product_id and
                        line.group_no == record.group_no and
                        (line.field_code == record.field_code or 
                         (not line.field_code and not record.field_code))
                    )
                )
                
                _logger.info("Found %d corresponding lines in picking", len(picking_lines))
                for line in picking_lines:
                    _logger.info("Will delete picking line %s (group_no=%s, field_code=%s)", 
                                line.id, line.group_no, line.field_code)
                    lines_to_delete_from_picking.append(line)
        
        # حذف السطور من المهام أولاً
        result = super().unlink()
        
        # حذف السطور المقابلة من stock.picking
        if lines_to_delete_from_picking:
            _logger.info("Deleting %d lines from stock.picking", len(lines_to_delete_from_picking))
            try:
                for line in lines_to_delete_from_picking:
                    if line.exists():
                        line.sudo().unlink()
                        _logger.info("Successfully deleted picking line %s", line.id)
            except Exception as e:
                _logger.error("Error deleting corresponding stock.move.line: %s", str(e))
