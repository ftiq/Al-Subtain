...
class StockPickingLabNotify(models.Model):
    ...
    def action_notify_lab(self):
        ...
        for picking in self:
            ...
            # يتطلب توقيعاً إلكترونياً قبل الإشعار
            if not getattr(picking, 'signature', False):
                raise UserError(_('يتطلب إشعار المختبر وجود توقيع إلكتروني على الاستلام. يرجى إضافة التوقيع في تبويب "التوقيع" أولاً.'))
