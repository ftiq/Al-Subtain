...
class StockPickingSamplingIntegration(models.Model):
    ...
    def _generate_or_update_samples(self):
        ...
        for current_group in range(1, total_samples + 1):
            # توليد رمز حقلي واحد لكل مجموعة
            ...
            # توليد رمز مختبري واحد للمجموعة (مشترك بين كل الأعمار)
            group_lab_code = self.env['stock.move.line']._generate_lab_code()

            if is_core:
                # للكور الخرساني: سطر واحد فقط بعمر فارغ وعدد مكعبات 2
                line_vals = {
                    'move_id': move.id,
                    'picking_id': self.id,
            ...
            # السلوك الافتراضي للأنواع الخرسانية الأخرى
            if enable_28_days:
                line_vals = {
                    'move_id': move.id,
                    'picking_id': self.id,
    ...
    def write(self, vals):
        ...
        # إذا تم تغيير نوع العينة إلى كور خرساني، طبّق الافتراضيات على السطور الحالية
        try:
