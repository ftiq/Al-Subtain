        ...
        AppointmentForm.include({
            ...
            async _onConfirmAppointment(event) {
                ...
                try {
                    ...
                    // 1) إضافة المنتجات إلى السلة (يمسح السلة أولاً ثم يضيف)
                await this._addProductsToCart();

                    // 2) ربط الطلب بالعميل الذى أنشأناه/حدثناه حتى لا يبقى Public user
                    try {
                        const orderInfo = await rpc('/appointment_products/get_sale_order', {});
                    ...
                    // رفع المرفقات بعد التأكد من وجود الطلب ومعرفه
                    try {
