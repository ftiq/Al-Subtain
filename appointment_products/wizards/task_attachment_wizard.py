...
class TaskAttachmentWizard(models.TransientModel):
    ...
    # الحقل الجديد لرفع عدة صور دفعة واحدة باستخدام واجهة many2many_binary
    ...
    def action_upload_images(self):
        ...
        # 1) الصور المرفوعة عبر الحقل الجديد (attachment_ids)
        images_data = []
        if self.attachment_ids:
            for att in self.attachment_ids:
                # "datas" يحتوي على الـ base64 للملف
                images_data.append((att.datas, att.name))

        # 2) دعم الحقول القديمة لضمان التوافق مع أي استعمال سابق
        legacy_images = [
        ...
        # أضف فقط الصور غير الفارغة من الحقول القديمة
    ...
    # ---------------------------------------------------------------------
    # حذف المرفقات المؤقتة بعد انتهاء عمر المعالج
    # ---------------------------------------------------------------------
