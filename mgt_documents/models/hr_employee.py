...
class HrEmployee(models.Model):
    ...
    def _compute_document_statistics(self):
        ...
        for employee in self:
            ...
            employee.approvals_pending_count = self.env['document.approval.request'].search_count([
                ('approver_id', '=', employee.id),
                ('status', '=', 'pending')
            ])
            

            employee.approvals_completed_count = self.env['document.approval.request'].search_count([
                ('approver_id', '=', employee.id),
