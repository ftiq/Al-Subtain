...
class ResUsers(models.Model):
    ...
    def get_accessible_documents(self):
        ...
        # Admin: full access
        if self.has_group('mgt_documents.group_doc_admin'):
            return Doc.search([])

        # Determine department id if any
        dept_id = self.employee_id.department_id.id if self.employee_id and self.employee_id.department_id else False

        # Manager/Director: broader access aligned with rules
        if self.has_group('mgt_documents.group_doc_manager'):
            domain = ['|', '|', '|',
                      ('create_uid', '=', self.id),
                      ('recipient_employee_id.user_id', '=', self.id),
                      ('department_id', 'child_of', dept_id or 0),
                      ('sender_employee_id.department_id', 'child_of', dept_id or 0)]
            return Doc.search(domain)

        # Processor/User: own, recipient, same department
        domain = ['|', '|',
                  ('create_uid', '=', self.id),
                  ('recipient_employee_id.user_id', '=', self.id),
