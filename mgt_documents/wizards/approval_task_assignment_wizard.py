        ...
        if self.assignment_type != 'none':
            ...
            if self.document_id:
                self.document_id.message_post(
                    body=f"""
                    <p><strong>تم إنشاء مهمة بناءً على موافقة:</strong></p>
                    <ul>
                        <li><strong>بيان الأمر:</strong> {self.official_directive}</li>
                        <li><strong>عنوان المهمة:</strong> {self.task_title}</li>
                        <li><strong>المُكلف:</strong> {self._get_assigned_description()}</li>
                        <li><strong>تاريخ الاستحقاق:</strong> {self.due_date}</li>
                    </ul>
                    """,
