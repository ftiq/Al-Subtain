...
class DashboardData(models.TransientModel):
    ...
    def get_dashboard_data(self):
        ...
        statistics = {
            ...
            'archived_total': len(documents.filtered(lambda d: getattr(d, 'workflow_state', False) == 'archived' or getattr(d, 'processing_state', False) == 'archived')),
            'draft_documents': len(documents.filtered(lambda d: getattr(d, 'workflow_state', False) == 'draft')),
            'submitted_documents': len(documents.filtered(lambda d: getattr(d, 'workflow_state', False) in ('submitted', 'in_review'))),
            'approved_documents': len(documents.filtered(lambda d: getattr(d, 'workflow_state', False) == 'approved')),
        ...
        for doc in recent_documents:
            recent_docs_data.append({
                ...
                'workflow_state': getattr(doc, 'workflow_state', False),
