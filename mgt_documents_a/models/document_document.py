...
class DocumentDocument(models.Model):
    ...
    def action_submit_for_processing(self):
        ...
        for doc in self:
            ...
            _logger.info(f"[DOC][submit_for_processing][start] user=%s doc_id=%s ref=%s ws=%s ps=%s",
                         doc.env.user.login, doc.id, doc.reference_number, doc.workflow_state, doc.processing_state)
            doc.write({'workflow_state': 'submitted', 'submitted_date': fields.Datetime.now()})
            _logger.info(f"[DOC][submit_for_processing][after_write] doc_id=%s ws=%s ps=%s",
                         doc.id, doc.workflow_state, doc.processing_state)
            doc._trigger_workflow()
            _logger.info(f"[DOC][submit_for_processing][after_trigger] doc_id=%s wf_id=%s ws=%s ps=%s",
                         doc.id, doc.workflow_id.id if doc.workflow_id else False, doc.workflow_state, doc.processing_state)
            # إنشاء طلب موافقة تلقائي للأنواع التي تتطلب ذلك
            if doc.document_type in ['outgoing', 'circular', 'memo']:
                try:
                    doc._create_approval_request()
                except Exception:
                    pass
    ...
    def action_start_review(self):
        ...
        for doc in self:
            ...
            _logger.info(f"[DOC][start_review][start] user=%s doc_id=%s ref=%s ws=%s ps=%s",
                         doc.env.user.login, doc.id, doc.reference_number, doc.workflow_state, doc.processing_state)
            doc.write({
            ...
            _logger.info(f"[DOC][start_review][done] doc_id=%s ws=%s ps=%s",
                         doc.id, doc.workflow_state, doc.processing_state)
    ...
    def action_complete(self):
        ...
        for doc in self:
            ...
            with doc.env.cr.savepoint():
                ...
                _logger.info(f"[DOC][complete][done] doc_id=%s ws=%s ps=%s",
                             doc.id, doc.workflow_state, doc.processing_state)
    ...
    def action_register(self):
        ...
        for doc in self:
            ...
            _logger.info(f"[DOC][register][start] user=%s doc_id=%s ref=%s ws=%s ps=%s",
                         doc.env.user.login, doc.id, doc.reference_number, doc.workflow_state, doc.processing_state)
            doc.write({
            ...
            _logger.info(f"[DOC][register][done] doc_id=%s ws=%s ps=%s",
                         doc.id, doc.workflow_state, doc.processing_state)
    ...
    def action_route(self):
        ...
        for doc in self:
            ...
            _logger.info(f"[DOC][route][start] user=%s doc_id=%s ref=%s ws=%s ps=%s dept=%s",
                         doc.env.user.login, doc.id, doc.reference_number, doc.workflow_state, doc.processing_state,
                         doc.department_id.display_name if doc.department_id else None)
            doc._trigger_workflow()
            _logger.info(f"[DOC][route][after_trigger] doc_id=%s wf_id=%s",
                         doc.id, doc.workflow_id.id if doc.workflow_id else False)
            ...
            _logger.info(f"[DOC][route][after_write] doc_id=%s ws=%s ps=%s",
                         doc.id, doc.workflow_state, doc.processing_state)
    ...
    def action_auto_route(self):
        """توجيه تلقائي للوثيقة عبر محرك التوجيه"""
        self.ensure_one()
        try:
            result = self.env['document.router'].process_document(self)
        except Exception:
            result = {'success': False, 'message': _('حدث خطأ أثناء تشغيل محرك التوجيه')}
        success = bool(result and result.get('success'))
        message = result.get('message') if isinstance(result, dict) else _('لم تنطبق أي قاعدة توجيه')
        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': _('نتيجة التوجيه') if success else _('لم تنطبق أي قاعدة توجيه'),
                'message': message,
    ...
    def action_approve(self):
        ...
        for doc in self:
            ...
            _logger.info(f"[DOC][approve][start] user=%s doc_id=%s ref=%s ws=%s ps=%s",
                         doc.env.user.login, doc.id, doc.reference_number, doc.workflow_state, doc.processing_state)
            doc.write({
            ...
            _logger.info(f"[DOC][approve][done] doc_id=%s ws=%s ps=%s",
                         doc.id, doc.workflow_state, doc.processing_state)
    ...
    def action_start_execution_processing(self):
        ...
        for doc in self:
            ...
            _logger.info(f"[DOC][start_execution_processing][start] user=%s doc_id=%s ref=%s ws=%s ps=%s",
                         doc.env.user.login, doc.id, doc.reference_number, doc.workflow_state, doc.processing_state)
            doc.write({
            ...
            _logger.info(f"[DOC][start_execution_processing][done] doc_id=%s ws=%s ps=%s",
                         doc.id, doc.workflow_state, doc.processing_state)
    ...
    def action_reject(self):
        ...
        for doc in self:
            ...
            _logger.info(f"[DOC][reject] user=%s doc_id=%s ref=%s old_ps=%s new_ps=%s ws=%s",
                         doc.env.user.login, doc.id, doc.reference_number, old_state, doc.processing_state, doc.workflow_state)
    ...
    def action_archive(self):
        ...
        for doc in self:
            ...
            _logger.info(f"[DOC][archive][start] user=%s doc_id=%s ref=%s ws=%s ps=%s",
                         doc.env.user.login, doc.id, doc.reference_number, doc.workflow_state, doc.processing_state)
            doc.write({
            ...
            _logger.info(f"[DOC][archive][done] doc_id=%s ws=%s ps=%s",
                         doc.id, doc.workflow_state, doc.processing_state)
    ...
    def _trigger_workflow(self):
        ...
        self.ensure_one()
        _logger.info(f"[DOC][trigger_wf][start] doc_id=%s ref=%s type=%s prio=%s dept=%s",
                     self.id, self.reference_number, self.document_type,
                     self.routing_priority, self.department_id.display_name if self.department_id else None)
        suitable_process = self._find_suitable_business_process()
        if not suitable_process:
            _logger.info(f"[DOC][trigger_wf][no_process] doc_id=%s", self.id)
            return
        ...
        if suitable_process:
            workflow_instance = suitable_process.create_instance(
                ...
                trigger_data={
                    ...
                    'category_id': self.category_id.id if hasattr(self, 'category_id') and self.category_id else False,
            ...
            if workflow_instance:
                ...
                _logger.info(f"[DOC][trigger_wf][instance_created] doc_id=%s wf_id=%s process=%s target=%s",
                             self.id, self.workflow_id.id if self.workflow_id else None,
                             suitable_process.display_name, self.target_completion_date)
            else:
                _logger.warning(f"[DOC][trigger_wf][create_failed] doc_id=%s process=%s",
                                self.id, suitable_process.display_name)
    ...
    def _find_suitable_business_process(self):
        ...
        category_mapping = {
            'incoming': 'document_review',
            'request': 'task_assignment',
            'internal': 'approval_workflow'
        }
        
        if self.document_type in category_mapping:
            domain.append(('category', '=', category_mapping[self.document_type]))
    ...
    def action_resume_from_hold(self):
        ...
        for doc in self:
            ...
            target_workflow = doc._map_processing_to_workflow_state(target_processing)
            old_ps, old_ws = doc.processing_state, doc.workflow_state
            doc.write({
                'processing_state': target_processing,
                'workflow_state': target_workflow,
            })
            doc._create_history_record('restored', _('تم استئناف المعالجة بعد التعليق'), old_ps, target_processing)
            _logger.info(f"[DOC][resume_from_hold] doc_id=%s ps:%s->%s ws:%s->%s", doc.id, old_ps, target_processing, old_ws, target_workflow)

    def _cleanup_runtime_links(self):
        """تنظيف الروابط وقت الرجوع لمسودة: إلغاء المهام غير المنتهية وفك ارتباط سير العمل"""
        for doc in self:
            active_tasks = doc.task_ids.filtered(lambda t: t.state not in ['completed', 'cancelled'])
            if active_tasks:
                active_tasks.write({'state': 'cancelled'})
            doc.workflow_id = False
            doc.current_step = False
    ...
    def action_reset_to_draft(self):
        ...
        for doc in self:
            ...
            _logger.info(f"[DOC][reset_to_draft] doc_id=%s ps:%s->received ws:%s->draft", doc.id, old_ps, old_ws)

    def action_uncancel(self):
        """إلغاء الإلغاء/الرفض وإرجاع الوثيقة لمسودة"""
        for doc in self:
            if doc.workflow_state != 'cancelled' and doc.processing_state != 'rejected':
                raise UserError(_('الوثيقة ليست ملغاة أو مرفوضة'))
            doc.action_reset_to_draft()

    def action_spawn_current_step_tasks(self):
        """إنشاء مهام للخطوة الحالية في سير العمل لهذه الوثيقة (إن لم تكن موجودة)."""
        self.ensure_one()
        if not self.workflow_id or not self.workflow_id.current_step_id:
            raise UserError(_('لا توجد خطوة حالية لسير العمل لتوليد مهام لها'))
        step = self.workflow_id.current_step_id
        # هل توجد مهام مسبقة لهذه الخطوة؟
        exists = self.env['admin.task'].search([
            ('process_instance_id', '=', self.workflow_id.id),
        ...
        if exists:
            return {
                ...
                'params': {
                    'title': _('تنبيه'),
                    'message': _('تم إنشاء مهام لهذه الخطوة مسبقاً.'),
                    'type': 'warning',
                    'sticky': False,
                }
            }
        # توليد المهام
        self.workflow_id._spawn_tasks_for_step(step)
        try:
            self.message_post(body=_('تم إنشاء مهام للخطوة: %s') % (step.name or ''))
        except Exception:
            pass
    ...
    def _create_history_record(self, action, description, old_state=None, new_state=None):
        ...
        try:
            self.env['document.history'].create({
                ...
                'new_state': new_state or self.workflow_state if hasattr(self, 'workflow_state') else False,
            })
        except Exception as e:
    ...
    def _map_processing_to_workflow_state(self, p_state):
        """تحويل حالة المعالجة إلى حالة سير العمل الأقرب"""
        mapping = {
            'received': 'draft',
            ...
            'rejected': 'cancelled',
            'archived': 'archived',
            'on_hold': self.workflow_state or 'in_review',
        }
        return mapping.get(p_state or 'received', 'draft')
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if not vals.get('reference_number') or vals.get('reference_number') == _('جديد'):
                ...
                seq_code = sequence_mapping.get(doc_type, 'document.other')
                ref = self.env['ir.sequence'].next_by_code(seq_code)
                vals['reference_number'] = ref or self._generate_reference_number(vals)
        ...
        for rec in records:
            try:
                rec._auto_trigger_workflow_on_create()
            except Exception:
                pass
        return records
    ...
    def action_view_related_documents(self):
        ...
        return {
            ...
            'context': {
                'search_default_group_by_workflow_state': 1,
            },
            'target': 'current',
        }
