...
class DocumentRouter(models.Model):
    ...
    def apply_to_document(self, document):
        ...
        if self.target_process_id:
            try:
                ...
                if instance:
                    # Link already set in create_instance; ensure document states are visible
                    try:
                        document.write({'processing_state': 'routed'})
                        # Keep workflow_state simple: move to in_review to reflect routing
                        if hasattr(document, 'workflow_state'):
                            document.workflow_state = 'in_review'
                        # Update target completion date if helper exists
                        if hasattr(document, '_update_target_completion_date'):
                            document._update_target_completion_date(self.target_process_id)
                    except Exception as we:
                        _logger.warning(f"Failed to write routed state on document {document.id}: {str(we)}")
    ...
    def auto_route_new_documents(self):
        """طريقة استدعاء مجدولة لتوجيه الوثائق الجديدة تلقائياً"""

        docs = self.env['document.document'].search([
            ('processing_state', 'in', ['received', 'registered']),
            ('workflow_state', 'in', ['draft', 'submitted'])
        ], limit=200)
        routed = 0
        for doc in docs:
            try:
                ...
                _logger.warning(f"Auto route failed for doc {doc.id}: {str(e)}")
                continue
        _logger.info(f"Auto route cron processed {len(docs)} documents, routed={routed}")
        return True
