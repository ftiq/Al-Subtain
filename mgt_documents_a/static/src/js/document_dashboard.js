...
export class DocumentDashboard extends Component {
    ...
    async openDocuments(documentType, domain = []) {
        ...
        const typeDomains = {
            ...
            // archived can be represented in workflow_state or processing_state
            'archived': ["|", ["workflow_state", "=", "archived"], ["processing_state", "=", "archived"]],
            'all': []
        };
    ...
    openAllDocumentsByState(state) {
        ...
        // map legacy state to actual fields
        let domainState;
        if (state === 'archived') {
            domainState = ["|", ["workflow_state", "=", "archived"], ["processing_state", "=", "archived"]];
        } else if (state === 'rejected') {
            domainState = ["|", ["workflow_state", "=", "cancelled"], ["processing_state", "=", "rejected"]];
        } else {
            domainState = ["workflow_state", "=", state];
        }
        const finalDomain = Array.isArray(domainState) && domainState[0] === '|' ? domainState : [domainState];
    ...
    openDocumentsByState(docType, state) {
        ...
        // map legacy 'rejected' to actual fields and AND with document_type
        let domain;
        if (state === 'archived') {
            domain = ['&', ["document_type", "=", docType], "|", ["workflow_state", "=", "archived"], ["processing_state", "=", "archived"]];
        } else if (state === 'rejected') {
            domain = ['&', ["document_type", "=", docType], "|", ["workflow_state", "=", "cancelled"], ["processing_state", "=", "rejected"]];
        } else {
            domain = [["document_type", "=", docType], ["workflow_state", "=", state]];
        }
