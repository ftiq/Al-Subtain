...
class ApprovalTaskAssignmentWizard(models.TransientModel):    
    ...
    def _prepare_task_values(self):
        ...
        # ربط المهمة بنسخة سير العمل الحالية وخطوتها إن وُجدت
        inst = self.document_id.workflow_id if self.document_id and self.document_id.workflow_id else False
        current_step_id = inst.current_step_id.id if inst and inst.current_step_id else False
        process_id = inst.workflow_process_id.id if inst and inst.workflow_process_id else False
        ...
        return {
            ...
            'process_id': process_id or False,
            'process_instance_id': inst.id if inst else False,
            'workflow_step_id': current_step_id or False,
